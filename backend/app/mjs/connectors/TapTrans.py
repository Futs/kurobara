...
class TapTrans(Connector):
    def __init__(self):
        super().__init__()
        self.base_url = "https://taptrans.com"
    ...
    async def get_series_data(self, seriesId):
        ...
        title_element = document.select_one(".comics-detail__info h1")
        if not title_element:
            return None
            
        title = title_element.text.strip()
        cover_element = document.select_one(".comics-detail__img img")
        cover_url = cover_element.get("src") if cover_element else ""
        
        chapters = []
        chapter_elements = document.select(".chapter-list__item")
        ...
        for chapter_element in chapter_elements:
            ...
            name_element = chapter_element.select_one(".chapter-list__item-title")
    ...
    async def get_chapter_data(self, seriesId, chapterId):
        chapter_url = f"{self.base_url}/comics/{seriesId}/{chapterId}"
