...
class HentaiWebtoon(Connector):
    def __init__(self):
        super().__init__()
        self.base_url = "https://hentaiwebtoon.com"
        
    async def get_series_data(self, seriesId):
        series_url = f"{self.base_url}/webtoon/{seriesId}"
        document = await self.fetch_document(series_url)
        
        if not document:
            return None
            
        title_element = document.select_one("h1.title")
    ...
    async def get_chapter_data(self, seriesId, chapterId):
        chapter_url = f"{self.base_url}/webtoon/{seriesId}/{chapterId}"
